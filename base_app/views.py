...
from base_app.contract_models.krd import (
    toshev,
    kzvs,
    agro,
    tander,
...
def home_filial(request, _filial_slug):
    ...
    context["menus"] = Menu.objects.filter(
        as_active=True, filial__id=context["filial"].id
    )
    return render(request, f"base_app/home.html", context=context)


def show_test(request, **kwargs):
    context["choice_contracts"] = Contracts.objects.filter(
        as_active=True, filial__id=context["filial"].id
    )
...
def show_contracts(request, **kwargs):
    __clear_context()
    context["menus"] = Contracts.objects.filter(
        as_active=True, filial__slug=context["filial"].slug
    )
    return render(request, f"base_app/home.html", context=context)
...
def show_operations(request, **kwargs):
    ...
    context["contract"] = Contracts.objects.get(
        filial__slug=context["filial"].slug, slug=kwargs["_contract_slug"]
    )
...
def event_load_file(request, **kwargs):
    if kwargs["_operation_slug"] == "comparison_stock_ka":
        ...
        try:
            context["result"] = dict_module[context["contract"].slug].start(
                _file, context["contract"]
            )
    ...
    if kwargs["_operation_slug"] == "check_goods_in_pg":
        _file = request.FILES["file"]
        context["result"] = PgGoods().get_goods_list_by_marking_goods(
            _file_marking_goods=_file, _contract=context["contract"]
        )
        try:
            return FileResponse(open(context["result"], "rb"))
        except Exception as e:
    ...
    if kwargs["_operation_slug"] == "build_peresort":
        _file = request.FILES["file"]
        context["result"] = ok.build_peresort(
            _file_name=_file, _contract=context["contract"]
        )
        try:
            return FileResponse(open(context["result"], "rb"))
        except Exception as e:
    ...
    if kwargs["_operation_slug"] == "add_goods":
        ...
        if file:
            res, error_valid = dict_module[context["contract"].slug].add_goods(
    ...
    if request.method == "POST":
        ...
        if file:
            if context["contract"].slug == "agrokompleks":
                res, error_valid = dict_module[context["contract"].slug].start(
                    file, kwargs
                )
            else:
                res, error_valid = dict_module[context["contract"].slug].start(
    ...
    if request.method == "POST":
        if kwargs["_operation_slug"] == "check_goods_in_pg":
            if "search_input" in request.POST:
                ...
                context["result"] = PgGoods().get_good_by_marking_goods(
                    _marking_goods=str(_marking_goods), _contract=context["contract"]
                )
            else:
                _file = request.FILES["file"]
                context["result"] = PgGoods().get_goods_list_by_marking_goods(
...
def event_load_stock(request, **kwargs):
    if kwargs["_operation_slug"] == "comparison-stock":
        __file_pg_stock = PgStocks().query_goods_stock_by_group_id(
            _contract=context["contract"]
        )
        __file_wms_stock = WmsStocks(
            _contract=context["contract"]
        ).get_goods_by_guid_group(_contract=context["contract"])
        context["result"] = comparison_stock(
            _contract=context["contract"],
            __file_pg_stock=__file_pg_stock,
    ...
    if request.method == "POST":
        if kwargs["_operation_slug"] == "load_stock_pg":
            context["result"] = PgStocks().query_goods_stock_by_group_id(
                _contract=context["contract"]
            )
        elif kwargs["_operation_slug"] == "load_stock_pg_ok":
            _file_name_list = PgStocks().query_goods_stock_by_group_id_ok(
                _contract=context["contract"]
            )
            _dict_file_name = {}
...
def show_reports_operation(request, **kwargs):
    print(kwargs["_reports_slug"])
    context["report"] = Reports.objects.get(slug=kwargs["_reports_slug"])
    context["contracts"] = Contracts.objects.filter(
        as_active=True, filial__slug=context["filial"].slug
    )
    from .forms import SverkiForm

    context["form"] = SverkiForm(choice_contracts=context["contracts"])
